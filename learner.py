...
def world_observe(oldworld, action, newworld, oldrules, oldnegrules, predictedworld=None):
    ...
    for changeset in changesets:
        for (y1_abs, x1_abs) in changeset:
            ...
            rule = (tuple(action_values_precondition), (0, 0, newworld[BOARD][y1_abs][x1_abs], tuple(
                [newworld[VALUES][0]-oldworld[VALUES][0]] + list(newworld[VALUES][1:]))))
...
def max_depth__breadth_first_search(world, rules, actions, max_depth=100, customGoal=None):
    # Initialize queue with world state, empty action list, and depth 0
    queue = deque([(world, [], 0)])
    encountered = dict([])
...
def airis_step(rulesin, negrules, oldworld):
    ...
    for i, rule1 in enumerate(rulesin):
        # exclude rules which are not better than exp (only 0.5+ makes sense here)
        if K.Truth_Expectation(K.TruthValue(K.RuleEvidence[rule1])) <= 0.5:
            if rule1 in rules:
                rulesExcluded.add(rule1)
                rules.remove(rule1)
        # exclude rules which are worse by truth value
    ...
    favoured_actions, airis_score, favoured_actions_for_revisit, oldest_age = max_depth__breadth_first_search(
        W.observed_world, rules, W.actions, customGoal=cupIsOnTable)
    debuginput = ""
    ...
    if airis_score >= 1.0 or babble or len(favoured_actions) == 0:
        if not babble and oldest_age > 0.0 and airis_score == 1.0 and len(favoured_actions_for_revisit) != 0:
            print("EXPLORE", P.plan_prettystring(
                favoured_actions_for_revisit), oldest_age)
    ...
    predicted_world, _, __ = world_predict(
        deepcopy(observed_world_old), action, rules)
    ...
    print("\033[0m")
    (newrules, newnegrules) = world_observe(observed_world_old,
                                            action, W.observed_world, rules, negrules, predicted_world)
