...
def RemoveRule(ruleset, negruleset, rule):
    for r in ruleVariants(rule):
        ...
        print("Neg. revised: ", end="");  P.prettyPrintRule(rule)
...
def AddRule(ruleset, negruleset, rule): #try location symmetry
    ...
    for rule in variants:
        ...
        print("Pos. revised: ", end="");  P.prettyPrintRule(rule)
...
def ruleVariants(rule): #location symmetry (knowledge about movement operations for faster learning)
    ...
    for (y,x,v) in conditions: #unnecessary
        ...
        if (action == W.up or action == W.down) and x != 0:
            return []
    rules = [rule]
    if action != W.left and action != W.right and action != W.down and action != W.up: #not such an op where symmetry would apply
        return rules
